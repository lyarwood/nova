...
class DriverVolumeBlockDevice(DriverBlockDevice):
    ...
    def _driver_detach(self, context, instance, connection_info, volume_api,
                       virt_driver):
        LOG.info(_LI('Attempting to driver detach volume %(volume_id)s from '
                  ' mountpoint %(mp)s'), {'volume_id': self.volume_id,
                  'mp': self['mount_device']}, instance=instance)
        ...
        encryption = encryptors.get_encryption_metadata(context,
                volume_api, self.volume_id, connection_info)
    ...
    def detach(self, context, instance, volume_api, virt_driver,
               attachment_id=None, do_driver_detach=True,
               do_volume_detach=True, destroy_bdm=False):
        ...
        if do_driver_detach:
            if CONF.host == instance.host:
                ...
                try:
                    self._driver_detach(context, instance, connection_info,
                ...
                LOG.debug("Skipping _driver_detach_volume during remote "
